<?php
	...
	class onedrive{
		...
		static $redirect_uri = "https://oneindex.github.io/";
			...
			if($token['expires_on'] > time()+600){
				...
				if(!empty($token['refresh_token'])){
					...
					$TokenData = array(
						...
						'up_time' => new Time('now','Asia/Shanghai')
					);
					$tokenModel->SetToken(1, $TokenData);
					return $token['access_token'];
				}
			}
			return "";
		...
		static function dir_next_page($request, &$items, $retry=0){
			...
			foreach((array)$data['value'] as $item){
				$items[$item['name']] = array(
					...
					'downloadUrl'=>empty($item['@microsoft.graph.downloadUrl'])?"1":$item['@microsoft.graph.downloadUrl'],
					'folder'=>empty($item['folder'])?false:true,
					'type'=>!empty($item['folder'])?"folder":".".substr(strrchr($item['name'], '.'), 1),
				);
			}
